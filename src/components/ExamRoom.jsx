...
export default function ExamRoom({ studentData, onFinish }) {
  ...
  // State untuk Trigger Kumpul Paksa dari Guru
  ...
  // 2. LISTENER REMOTE CONTROL DARI GURU (DISPENSASI & KUMPUL PAKSA)
  useEffect(() => {
    if (!sid || sid === 'guest') return;
    const studentRef = ref(db, `live_students/${sid}`);
    const unsub = onValue(studentRef, (snap) => {
      if (snap.exists()) {
        const data = snap.val();
        
        // A. Fitur Buka Kunci / Dispensasi
        if (data.warnings === 0 && isLocked) {
          setWarnings(0);
          setIsLocked(false);
          localStorage.setItem(`${storageKey}_warn`, 0);
          localStorage.setItem(`${storageKey}_lock`, 'false');
          alert("PEMBERITAHUAN!\nPengawas telah memberikan dispensasi. Layar Anda telah dibuka kembali.");
        }
        
        // B. Fitur Kumpul Paksa
        if (data.forceSubmit === true) {
  ...
  useEffect(() => {
    ...
    // Trigger submit jika waktu habis ATAU disuruh guru (forceSubmit)
    else if ((timeLeft <= 0 || shouldForceSubmit) && questions.length > 0) {
      submitExam();
    }
  ...
  const submitExam = async () => {
    const finalAnswers = answersRef.current; // Menggunakan ref agar nilainya selalu yang paling baru
    let correct = 0; 
  ...
  if (isLocked) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-red-50 p-6 text-center">
        ...
        <p className="text-sm text-red-500 mt-2">Menyegarkan halaman tidak akan membuka kunci ini. Silakan panggil Pengawas Ruangan.</p>
      </div>
    );
  }

  // === RENDER LAYAR BUKAN FULLSCREEN ===
