...
export default function TeacherDashboard({ onLogout }) {
  ...
  // PERBAIKAN: Mengubah status jadi force_finish agar dibaca oleh sistem Siswa
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-950 dark:to-emerald-900 text-gray-900 dark:text-gray-100 font-sans relative">
      ...
      <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden fixed bottom-6 left-4 p-3 bg-emerald-600 text-white rounded-full shadow-lg z-30 print:hidden">
        <Menu size={24} />
      </button>

      {isSidebarOpen && <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 lg:hidden" onClick={() => setIsSidebarOpen(false)} />}

      <div className={`fixed inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition-transform duration-300 ease-in-out z-50 w-72 bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl shadow-2xl lg:shadow-[4px_0_24px_rgba(0,0,0,0.05)] print:hidden border-r border-white/50 dark:border-gray-700/50 flex flex-col`}>
        <div className="p-6 md:p-8 border-b border-emerald-100 dark:border-gray-800 flex justify-between items-center">
          <div className="text-center w-full">
            <div className="w-14 h-14 md:w-16 h-16 mx-auto bg-gradient-to-tr from-emerald-500 to-green-400 rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-500/30 mb-3 text-white font-black text-xl md:text-2xl">CBT</div>
            <h2 className="text-lg md:text-xl font-black text-emerald-800 dark:text-emerald-400 tracking-wide">Portal Guru</h2>
          </div>
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 text-gray-500 absolute top-4 right-4 bg-gray-100 dark:bg-gray-800 rounded-full"><X size={20} /></button>
        </div>
        <nav className="flex-1 p-4 md:p-5 space-y-2 md:space-y-3 overflow-y-auto">
          <button onClick={() => {setActiveTab('proctor'); setIsSidebarOpen(false)}} className={`w-full flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl font-bold transition-all ${activeTab === 'proctor' ? 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/30' : 'hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300'}`}><Users size={20} /> Pantau Ujian</button>
          <button onClick={() => {setActiveTab('bank'); setIsSidebarOpen(false)}} className={`w-full flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl font-bold transition-all ${activeTab === 'bank' ? 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/30' : 'hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300'}`}><BookOpen size={20} /> Bank Soal</button>
          <button onClick={() => {setActiveTab('rekap'); setIsSidebarOpen(false)}} className={`w-full flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl font-bold transition-all ${activeTab === 'rekap' ? 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/30' : 'hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300'}`}><Award size={20} /> Rekap Nilai</button>
          <button onClick={() => {setActiveTab('settings'); setIsSidebarOpen(false)}} className={`w-full flex items-center gap-3 p-3 md:p-4 rounded-xl md:rounded-2xl font-bold transition-all ${activeTab === 'settings' ? 'bg-gradient-to-r from-emerald-500 to-green-600 text-white shadow-lg shadow-emerald-500/30' : 'hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300'}`}><Settings size={20} /> Kelola Sesi</button>
        </nav>
        <div className="p-4 md:p-5">
          <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 p-3 md:p-4 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white dark:bg-red-900/20 dark:hover:bg-red-600 rounded-xl md:rounded-2xl font-black transition shadow-sm"><LogOut size={18} /> Keluar</button>
        </div>
      </div>

      <div className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden p-4 md:p-6 lg:p-10 w-full pb-20 md:pb-10">
        <header className="bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl shadow-sm rounded-2xl md:rounded-3xl p-4 md:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center print:hidden border border-white/50 dark:border-gray-800 mb-6 md:mb-8 gap-3">
          <h1 className="text-xl md:text-2xl font-black text-emerald-900 dark:text-emerald-100 ml-10 lg:ml-0">Dasbor Guru</h1>
          <div className="px-4 py-2 text-xs md:text-sm bg-gradient-to-r from-emerald-100 to-green-100 dark:from-emerald-900/60 dark:to-green-900/60 text-emerald-800 dark:text-emerald-300 rounded-full font-black border border-emerald-200 dark:border-emerald-700 shadow-sm truncate w-full sm:w-auto text-center sm:text-left">
        ...
        <main>
                   ...
                   <tbody className="divide-y dark:divide-gray-700 text-sm md:text-base">
                     {liveStudents.map(student => (
                       <tr key={student.id}>
                         ...
                         <td className="p-4 md:p-5">
                            <span className={`px-2 md:px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap ${student.status === 'force_finish' ? 'bg-amber-100 text-amber-700' : student.status === 'selesai' ? 'bg-gray-200 text-gray-700' : 'bg-emerald-100 text-emerald-700'}`}>
                                {student.status === 'force_finish' ? 'Memproses...' : student.status}
                            </span>
                         </td>
                         <td className="p-4 md:p-5 text-center">
                            <button onClick={() => forceFinish(student.id)} disabled={student.status === 'selesai' || student.status === 'force_finish'} className="text-red-500 hover:text-white hover:bg-red-500 px-2 md:px-3 py-1 rounded-lg text-xs md:text-sm font-bold transition whitespace-nowrap disabled:opacity-30">
                                Paksa Selesai
                            </button>
                         </td>
                       </tr>
            ...
            <div className="print:hidden">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-3">
                <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                  <button onClick={handleDownloadExcel} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-white/70 dark:bg-gray-800/70 backdrop-blur-md border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 px-3 md:px-5 py-2 md:py-2.5 rounded-xl font-bold hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition shadow-sm text-xs md:text-sm"><Download size={16}/> Template</button>
                  <input type="file" accept=".xlsx, .xls" className="hidden" ref={fileInputRef} onChange={handleUploadExcel} />
                  <button onClick={() => fileInputRef.current.click()} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 px-3 md:px-5 py-2 md:py-2.5 rounded-xl font-bold hover:bg-amber-200 dark:hover:bg-amber-900/50 transition shadow-sm text-xs md:text-sm"><Upload size={16}/> Upload</button>
                </div>
                <button onClick={() => { setEditId(null); setFormData({ pertanyaan: '', opsiA: '', opsiB: '', opsiC: '', opsiD: '', kunci: 'A' }); setShowModal(true); }} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white px-4 md:px-6 py-2.5 md:py-3 rounded-xl shadow-lg shadow-emerald-500/30 font-black transition transform hover:-translate-y-1 text-sm md:text-base"><Plus size={18}/> Buat Manual</button>
              </div>
              ...
              <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl md:rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 dark:border-gray-700 divide-y dark:divide-gray-700">
                {bankSoal.length === 0 && <p className="p-8 text-center text-emerald-600/50 font-bold text-sm md:text-lg">Belum ada soal. Bank Soal masih bersih.</p>}
                {bankSoal.map((soal, idx) => (
                  <div key={soal.id} className="p-4 md:p-8 flex flex-col md:flex-row justify-between items-start hover:bg-white/90 dark:hover:bg-gray-700/50 transition gap-4">
                    <div className="w-full">
                      <p className="font-bold text-base md:text-xl mb-3 md:mb-4 text-emerald-900 dark:text-emerald-100"><span className="text-emerald-500 bg-emerald-50 dark:bg-emerald-900/50 px-2 py-1 rounded-lg mr-2">{idx + 1}.</span> {soal.pertanyaan}</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-2 text-emerald-800/80 dark:text-emerald-200/80 font-medium text-xs md:text-base">
                      ...
                      <p className="mt-4 inline-flex items-center gap-1.5 bg-gradient-to-r from-emerald-100 to-green-100 dark:from-emerald-900/50 dark:to-green-900/50 text-emerald-700 dark:text-emerald-300 px-3 py-1.5 rounded-lg font-black text-xs md:text-sm border border-emerald-200 dark:border-emerald-800"><CheckCircle size={14}/> Kunci: {soal.kunci}</p>
                    </div>
                    <div className="flex md:flex-col gap-2 w-full md:w-auto justify-end mt-2 pt-4 md:pt-0 border-t md:border-0 border-gray-100 dark:border-gray-700">
                      <button onClick={() => { setEditId(soal.id); setFormData(soal); setShowModal(true); }} className="flex-1 md:flex-none text-center text-emerald-600 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:hover:bg-emerald-800/50 px-3 py-2 rounded-xl font-bold transition text-xs md:text-sm">Edit</button>
                      <button onClick={() => handleDeleteSoal(soal.id)} className="flex items-center justify-center text-red-500 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 p-2 md:p-2.5 rounded-xl transition"><Trash2 size={16} className="md:w-5 md:h-5"/></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'rekap' && (
             <div className="print:block">
               <div className="mb-4 md:mb-6 flex justify-end print:hidden">
                 <button onClick={() => window.print()} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-green-700 text-white px-5 py-2.5 md:py-3 rounded-xl font-bold shadow-lg text-sm md:text-base"><Printer size={16}/> Cetak PDF</button>
               </div>
               
               <div className="bg-white/70 dark:bg-gray-800/70 rounded-2xl md:rounded-3xl shadow-sm border border-white/50 dark:border-gray-700 overflow-x-auto backdrop-blur-md w-full">
          ...
          {activeTab === 'settings' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 print:hidden">
              ...
              <div className="md:col-span-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl md:rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 dark:border-gray-700 overflow-hidden w-full">
                  ...
                  <table className="w-full text-left min-w-[500px]">
                    <tbody className="divide-y divide-emerald-50 dark:divide-gray-700 text-sm md:text-base">
                      ...
                      <tr key={s.id} className="hover:bg-white/50 dark:hover:bg-gray-700/30">
                        ...
                        <td className="p-4 md:p-5 text-right"><button onClick={() => deleteSession(s.id)} className="p-2 md:p-3 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl"><Trash2 size={16} className="md:w-5 md:h-5"/></button></td>
                      </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-emerald-950/40 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl p-5 md:p-8 rounded-2xl md:rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl border border-white/50 dark:border-emerald-800/50">
            ...
            <form onSubmit={handleSaveSoal} className="space-y-4 md:space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-3 md:gap-4 pt-4 md:pt-6 mt-2 md:mt-4 border-t border-emerald-100 dark:border-gray-800">
                ...
                <button type="submit" className="px-6 md:px-8 py-2.5 md:py-3 bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-xl font-black shadow-lg shadow-emerald-500/40 transform hover:-translate-y-1 transition text-sm md:text-base w-full sm:w-auto">Simpan</button>
              </div>
            </form>
          </div>
        </div>
