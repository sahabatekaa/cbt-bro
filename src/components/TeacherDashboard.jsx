...
export default function TeacherDashboard({ onLogout }) {
  ...
  // Filter Data khusus untuk Guru yang sedang Login
  ...
  // Ekstrak List Mapel & Kelas dari Soal untuk Form Sesi
  ...
  const downloadTemplate = () => {
    const template = [
      { mapel: "Bahasa Arab", kelas: "Tk.9", pertanyaan: "Contoh Soal", opsiA: "A", opsiB: "B", opsiC: "C", opsiD: "D", kunci: "A" }
    ];
  ...
  const handleCreateSession = (e) => {
    ...
    if (!token || !mapel || !kelas) return alert("Pilih Mapel dan Kelas terlebih dahulu!");
    if (sessions.find(s => s.token === token)) return alert("Token sudah digunakan, ganti yang lain!");
    ...
    alert("Berhasil! Sesi Ujian telah dibuka untuk Siswa.");
  };
  ...
  const deleteSession = (id, token) => {
    if(window.confirm(`Yakin ingin menghapus sesi ${token}?`)) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      {/* CSS KHUSUS PRINT LENGKAP KOP SURAT */}
      <style>
        {`
          @media print {
            @page { margin: 1cm; }
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-100 flex flex-col transition-transform duration-300 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 print:hidden`}>
        ...
        <div className="p-4 border-b border-gray-100">
          <p className="text-xs font-bold text-gray-400 uppercase mb-1">Login Guru</p>
          <p className="text-sm font-bold text-slate-800 truncate">{currentUserEmail}</p>
        </div>
          ...
          {activeTab === 'proctor' && (
            <div className="space-y-6">
              <div className="bg-emerald-100 text-emerald-800 p-4 rounded-xl flex items-center justify-between gap-2 border border-emerald-200">
                <div className="flex items-center gap-3 font-bold"><Eye size={22} className="text-emerald-600" /><span>{activeMonitorToken ? `Sesi Terpantau: ${activeMonitorToken}` : 'Pilih Sesi untuk Dimonitor'}</span></div>
                {mySessions.length > 0 && (
              ...
              {!activeMonitorToken ? (
                <div className="bg-white p-10 rounded-2xl border border-dashed border-gray-300 text-center flex flex-col items-center"><Filter size={48} className="mb-4 text-gray-300" /><h3 className="font-bold text-gray-700 text-lg">Menunggu Token</h3><p className="text-gray-500 text-sm mt-1">Ganti opsi di atas untuk mulai melihat data siswa yang online.</p></div>
              ) : (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-white p-6 rounded-2xl border-l-4 border-l-emerald-400 border border-gray-100 shadow-sm"><p className="text-gray-500 text-sm font-bold mb-1">Telah Selesai</p><p className="text-4xl font-black text-emerald-600">{monitoredStudents.filter(s => s.status === 'Selesai').length}</p></div>
                    <div className="bg-white p-6 rounded-2xl border-l-4 border-l-red-400 border border-gray-100 shadow-sm"><p className="text-gray-500 text-sm font-bold mb-1">Peringatan (Pindah Tab)</p><p className="text-4xl font-black text-red-600">{monitoredStudents.filter(s => s.warnings > 0).length}</p></div>
                  </div>
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <table className="w-full text-left"><thead className="border-b border-gray-100 bg-gray-50/50"><tr><th className="p-4 font-bold text-gray-800">Nama Siswa</th><th className="p-4 font-bold text-gray-800">Identitas</th><th className="p-4 font-bold text-gray-800 w-1/4">Progress Jawaban</th><th className="p-4 font-bold text-gray-800 text-right">Aksi</th></tr></thead>
                      <tbody className="divide-y divide-gray-50">
                        {monitoredStudents.map(s => (
                          <tr key={s.id} className="hover:bg-gray-50">
                            <td className="p-4"><p className="font-bold text-slate-800">{s.name}</p>{s.status === 'Pindah Tab' && <span className="text-xs text-red-600 font-bold animate-pulse">Melanggar {s.warnings}x</span>}</td>
                            <td className="p-4"><p className="font-bold text-slate-800 mb-1">{s.mapel || '-'}</p><span className="bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full text-xs font-bold inline-flex">{s.class}</span></td>
                            <td className="p-4"><div className="flex items-center gap-2"><div className="flex-1 bg-gray-100 h-2.5 rounded-full"><div className="bg-emerald-500 h-full" style={{ width: `${s.progress}%` }}></div></div><span className="text-sm font-bold text-gray-600 w-10 text-right">{s.progress}%</span></div></td>
                            <td className="p-4 text-right"><button onClick={() => update(ref(db, `live_students/${s.id}`), { status: 'Selesai' })} disabled={s.status === 'Selesai'} className="text-xs bg-gray-100 hover:bg-red-100 text-gray-600 hover:text-red-700 px-3 py-2 rounded-lg font-bold disabled:opacity-30">Paksa Kumpul</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit">
                ...
                <form onSubmit={handleCreateSession} className="space-y-4">
                  <div><label className="text-sm font-bold text-gray-500 mb-1 block">Mata Pelajaran</label><select id="mapel_session" required className="w-full p-3 rounded-xl border bg-gray-50 outline-none"><option value="">-- Pilih --</option>{availableMapel.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                  <div><label className="text-sm font-bold text-gray-500 mb-1 block">Tingkatan Kelas</label><select id="kelas_session" required className="w-full p-3 rounded-xl border bg-gray-50 outline-none"><option value="">-- Pilih --</option>{availableKelas.map(k => <option key={k} value={k}>{k}</option>)}</select></div>
                  <div><label className="text-sm font-bold text-gray-500 mb-1 block">Token Unik</label><div className="flex gap-2"><input id="token_input" type="text" required placeholder="Generate..." className="flex-1 p-3 rounded-xl border font-mono font-bold uppercase" /><button type="button" onClick={() => { document.getElementById('token_input').value = Math.random().toString(36).substring(2, 7).toUpperCase(); }} className="bg-emerald-100 text-emerald-700 p-3 rounded-xl"><Dices size={24} /></button></div></div>
                  <button type="submit" className="w-full bg-emerald-600 text-white py-3.5 rounded-xl font-bold active:scale-95">Rilis ke Siswa</button>
                </form>
              </div>
              <div className="lg:col-span-2 space-y-4">
                <h3 className="text-lg font-bold mb-4">Riwayat Sesi Dibuat</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {mySessions.map((s) => (
                    <div key={s.id} className={`p-5 rounded-2xl border ${s.status === 'open' ? 'bg-white border-emerald-200 shadow-sm' : 'bg-gray-50 border-gray-200'}`}>
                      <div className="flex justify-between items-start mb-4"><div><p className="font-mono text-3xl font-black mt-2 text-slate-800">{s.token}</p><div className="flex gap-2 mt-2"><span className="bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded font-bold mr-1">{s.mapel}</span><span className="bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded font-bold">{s.kelas}</span></div></div><span className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 ${s.status === 'open' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{s.status === 'open' ? <Unlock size={14}/> : <Lock size={14}/>}</span></div>
          ...
          {activeTab === 'bank' && (
            <div className="space-y-6 max-w-5xl mx-auto">
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setShowModal(true)} className="flex-1 sm:flex-none bg-emerald-600 text-white px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 font-medium"><Plus size={18}/> Buat Soal</button>
                ...
                <button onClick={() => fileInputRef.current.click()} className="flex-1 sm:flex-none bg-white border px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 font-medium text-emerald-700 border-emerald-200"><Upload size={18}/> Import Excel</button>
              </div>
              <div className="grid grid-cols-1 gap-4">
                {myQuestions.map((q, idx) => (
                  <div key={q.id} className="bg-white p-6 rounded-2xl border border-gray-100 flex gap-6 shadow-sm"><div className="flex-1"><div className="flex gap-2 mb-3"><span className="bg-emerald-50 text-emerald-800 border border-emerald-100 text-xs px-2.5 py-1 rounded font-bold">{q.mapel}</span><span className="bg-gray-100 text-gray-700 text-xs px-2.5 py-1 rounded font-bold">{q.kelas}</span></div><p className="font-bold text-slate-800 mb-4 text-lg">{idx + 1}. {q.pertanyaan}</p><div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-slate-700"><div className={`p-2 rounded-lg border ${q.kunci==='A'?'bg-emerald-50 text-emerald-900 font-bold border-emerald-200':'border-gray-100'}`}>A. {q.opsiA}</div><div className={`p-2 rounded-lg border ${q.kunci==='B'?'bg-emerald-50 text-emerald-900 font-bold border-emerald-200':'border-gray-100'}`}>B. {q.opsiB}</div><div className={`p-2 rounded-lg border ${q.kunci==='C'?'bg-emerald-50 text-emerald-900 font-bold border-emerald-200':'border-gray-100'}`}>C. {q.opsiC}</div><div className={`p-2 rounded-lg border ${q.kunci==='D'?'bg-emerald-50 text-emerald-900 font-bold border-emerald-200':'border-gray-100'}`}>D. {q.opsiD}</div></div></div><div className="flex items-center justify-center border-l pl-6"><button onClick={() => remove(ref(db, `bank_soal/${q.id}`))} className="text-red-500 bg-red-50 p-3 rounded-xl"><Trash2 size={20}/></button></div></div>
                ))}
              </div>
            </div>
          )}

          {/* TAB: REKAP NILAI & PRINT */}
          {activeTab === 'recap' && (
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 max-w-5xl mx-auto print:shadow-none print:border-none print:p-0">
              <div className="flex justify-between items-center mb-8 print:hidden">
                <h3 className="text-xl font-bold">Rekapitulasi Nilai Akhir</h3>
                <button onClick={() => window.print()} className="bg-emerald-600 text-white px-6 py-2.5 rounded-xl flex items-center gap-2 font-bold"><Download size={18}/> Cetak Laporan</button>
              </div>
              ...
              <div className="hidden print:block mb-8 border-b-4 border-double border-black pb-4 text-center">
                ...
                <p className="text-sm text-gray-800 mt-2">Daftar Rekapitulasi Nilai Siswa</p>
              </div>
              ...
              <table className="w-full text-left print:mt-4">
                <thead>
                  <tr className="border-b-2 border-gray-100 print:border-black bg-gray-50/50 print:bg-gray-100">
                    ...
                    <th className="p-4 font-bold text-gray-800">Nama Lengkap Siswa</th>
                    <th className="p-4 font-bold text-gray-800">Tingkat / Kelas</th>
                    <th className="p-4 font-bold text-gray-800 text-right">Skor Akhir</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 print:divide-black">
                  ...
                  {leaderboard.length === 0 && <tr><td colSpan="4" className="text-center p-10 text-gray-500">Belum ada data nilai ujian masuk.</td></tr>}
                </tbody>
              </table>
              ...
              <div className="hidden print:flex justify-end mt-16 pt-8">
                <div className="text-center">
                  ...
                  <p className="text-black font-bold border-b border-black inline-block min-w-[200px] pb-1">{currentUserEmail}</p>
                </div>
              </div>
            </div>
          )}
